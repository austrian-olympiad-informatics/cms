...
class StupidSandbox:
    ...
    is not secure against things like box escaping and fork bombs. Yet,
    it is very portable and has no dependencies, so it's very useful for
    testing on platforms where isolate is not available (e.g., arm64).
    Using in real contests is strongly discouraged.
    ...
    def __init__(self, name: str | None = None, temp_dir: str | None = None):
        """Initialization.

        name: name of the sandbox, used in log messages.
        temp_dir: temporary directory to use; if None, use the default
            temporary directory specified in the configuration.
        ...
        # Box parameters
        self.chdir: str = self._path
        self.stdin_file: str | None = None
        self.stdout_file: str | None = None
        self.stderr_file: str | None = None
        self.stack_space: int | None = None
        self.address_space: int | None = None
        self.timeout: float | None = None
        self.wallclock_timeout: float | None = None
        self.extra_timeout: float | None = None
    ...
    def get_stats(self) -> str:
        """Return a human-readable string with execution time and memory."""
        execution_time = self.get_execution_time()
        time_str = ("%.3f sec" % execution_time) if execution_time is not None \
            else "(time unknown)"
        return "[%s - (memory usage unknown)]" % time_str

    # TODO - returns wall clock time because CPU time is not available
    def get_execution_time(self) -> float | None:
        """Return the time spent in the sandbox."""
        return self.get_execution_wall_clock_time()

    def get_execution_wall_clock_time(self) -> float | None:
        """Return the total time from sandbox start to conclusion."""
        if self.exec_time:
    ...
    def get_memory_used(self) -> int | None:
        """Return the memory used by the sandbox (always None here)."""
        return None

    def get_killing_signal(self) -> int:
        """Return the signal that killed the sandboxed process, or 0."""
    ...
    def get_human_exit_description(self) -> str:
        """Return a human-readable description of why the sandbox terminated."""
        status = self.get_exit_status()
        if status == self.EXIT_OK:
    ...
    # --- File management (mirrors Sandbox, adapted to self._path) ---

    def create_file(self, path: str, executable: bool = False) -> typing.BinaryIO:
        """Create an empty file in the sandbox and open it in write binary mode."""
        real_path = self.relative_path(path)
        file_fd = os.open(real_path, os.O_CREAT | os.O_EXCL | os.O_WRONLY)
        file_ = open(file_fd, "wb")
        mod = stat.S_IRUSR | stat.S_IRGRP | stat.S_IROTH | stat.S_IWUSR
    ...
    def create_file_from_storage(
        self, path: str, digest: str, file_cacher: FileCacher,
    ...
    def get_file_to_string(self, path: str, maxlen: int | None = 1024) -> bytes:
        """Return the content of a file in the sandbox."""
        with self.get_file(path) as file_:
            if maxlen is None:
                return file_.read()
            else:
    ...
    def get_file_to_storage(
        self, path: str, file_cacher: FileCacher,
        description: str = "", trunc_len: int | None = None
    ) -> str:
        """Put a sandbox file in FS and return its digest."""
        with self.get_file(path, trunc_len=trunc_len) as file_:
            return file_cacher.put_file_from_fobj(file_, description)
    ...
    def file_exists(self, path: str) -> bool:
        """Return whether a file exists in the sandbox."""
        return os.path.exists(self.relative_path(path))
    ...
    def _popen(
        ...
        """Execute the given command via subprocess.Popen."""
        self.exec_time = None
        self.exec_num += 1

        logger.debug("Executing program in sandbox with command: `%s'.",
                     " ".join(command))
        with open(self.relative_path(self.cmd_file), "at",
                  encoding="utf-8") as commands:
    ...
    def execute_without_std(
        ...
        """Execute the given command, discarding standard streams."""

        def preexec_fn(sandbox):
            """Set resource limits for the child process."""
            if sandbox.chdir:
                os.chdir(sandbox.chdir)
            if sandbox.timeout:
                rlimit_cpu = sandbox.timeout
                if sandbox.extra_timeout:
                    rlimit_cpu += sandbox.extra_timeout
                rlimit_cpu = int(rlimit_cpu) + 1
                resource.setrlimit(resource.RLIMIT_CPU,
                                   (rlimit_cpu, rlimit_cpu))
            if sandbox.address_space:
                resource.setrlimit(resource.RLIMIT_DATA,
                                   (sandbox.address_space, sandbox.address_space))
            if sandbox.stack_space:
                resource.setrlimit(resource.RLIMIT_STACK,
                                   (sandbox.stack_space, sandbox.stack_space))
        ...
        self.popen = self._popen(
            ...
            stdout=stdout_fd,
            stderr=stderr_fd,
            preexec_fn=functools.partial(preexec_fn, self),
            close_fds=True)
    ...
    def translate_box_exitcode(self, exitcode: int) -> bool:
        """Translate the process exit code to a boolean success value.

        StupidSandbox always reports success from the sandbox perspective.
        """
        return True
