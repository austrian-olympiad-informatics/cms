...
# AOI: build our patched isolate fork from source instead of apt-installing it.
# Upstream `apt install isolate` only ships an amd64 build; building from source
# keeps the dev environment working natively on arm64 (Apple Silicon). Built on
# the same base image so the binary is glibc-compatible with the final stage.
FROM ${BASE_IMAGE} AS isolate-builder
...
# AOI: install our source-built isolate in place of upstream's apt package.
# We recreate what the isolate .deb would provide: the setuid binary, the
# `isolate` group (gid 1001, referenced by the cmsuser setup below), the config
# file, and the cgroup v2 root. Our isolate reads its config from
# /usr/local/etc/isolate (compiled-in CONFIG_FILE).
